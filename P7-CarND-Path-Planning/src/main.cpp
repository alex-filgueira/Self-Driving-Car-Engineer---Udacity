...
int main() {
 ...
 double speed_ref = 0.0;
  ...
  h.onMessage([&speed_ref, &next_car_road_lane,&lane_state, &map_waypoints_x,&map_waypoints_y,&map_waypoints_s,
    ...
    if (length && length > 2 && data[0] == '4' && data[1] == '2') {
      ...
      if (s != "") {
        ...
        if (event == "telemetry") {
            ...
            if(road_lane>=0){
              ...
              if(road_lane == (car_road_lane-1)){
                ...
                if(nextS > car_nextS+10){//In front
                  ...
                  if((nextS - car_nextS)<(d_security*1.4)){
                    flag_left = true;
              ...
              if(road_lane == (car_road_lane+1)){
                ...
                if(nextS > car_nextS+10){//in front
                  ...
                  if(nextS - car_nextS<(d_security*1.4)){//1.8
                    flag_right = true;
          ...
          double speed_dif = 0;
          bool flag_gas = false;
          bool flag_brake = false;

          //new state machine
          //lane_state, start in 0
          switch(lane_state) {
            case 0://in line
              if(!flag_front){
                flag_gas = true;
                ...
                if(!flag_right && !flag_left && (car_road_lane == 1)){
                  if(dToRight >= dToLeft){
                    lane_state = 1;
                    next_car_road_lane = car_road_lane + 1;
                  }
                  else{
                    lane_state = 2;
                    next_car_road_lane = car_road_lane - 1;
                ...
                else{//not in midle + 2 frees
                  if(((car_road_lane == 0) || (car_road_lane == 1)) && !flag_right){
                    lane_state = 1;
                    next_car_road_lane = car_road_lane + 1;
                  }
                  else if(((car_road_lane == 2) || (car_road_lane == 1)) && !flag_left){
                    lane_state = 2;
                    next_car_road_lane = car_road_lane - 1;
                  }
                  else{//stop
                    if(1.2*speed_ref > speed_front){
                    	flag_brake = true;
                    }
                    if(dToFront < 20){flag_brake = true;}
                  }
                }
              }
              break;
            case 1://turn right
              if((next_car_road_lane == 0) && (car_d >1.5)&& (car_d <2.5)){lane_state = 0;}
              if((next_car_road_lane == 1) && (car_d >5.5)&& (car_d <6.5)){lane_state = 0;}
              if((next_car_road_lane == 2) && (car_d >9.5)&& (car_d <10.5)){lane_state = 0;}
              if(flag_front){
                if(dToFront < 20){
                  if((next_car_road_lane == 0) && (car_d >0)&& (car_d <3.9)){flag_brake = true;}
                  if((next_car_road_lane == 1) && (car_d >4.1)&& (car_d <7.9)){flag_brake = true;}
                  if((next_car_road_lane == 2) && (car_d >8.1)&& (car_d <12)){flag_brake = true;}
                }
              }
              break;
            case 2://turn left
          ...
          cout << "lane_state: "<<lane_state<< " flag_gas: "<<flag_gas<< " flag_brake: "<<flag_brake<<endl;
          //gas or breake
          if(flag_gas){
          ...
          if(flag_brake){
            speed_dif = speed_dif - max_acc;// -gas
            //if(dToFront < 20){speed_dif = speed_dif - max_acc;}//dToFront = 10 -> impact!
            //if(dToFront < 15){speed_dif = speed_dif - max_acc;}
          }
